...
// Banco de dados: usar PostgreSQL se DATABASE_URL existir, senão SQLite local
let db;
...
if (usingPostgres) {
    ...
    db = {
        run: (sql, params, cb) => {
            const needsReturning = /^\s*INSERT\b/i.test(sql);
            const sqlRun = needsReturning ? (sql + ' RETURNING id') : sql;
            pool.query(sqlRun, params || [])
                .then(res => {
                    if (cb) cb.call({ lastID: res.rows[0] ? res.rows[0].id : null }, null);
                })
    ...
    // Criar tabelas compatíveis com PostgreSQL
    (async () => {
        try {
            await pool.query(`
            ...
            const adminRes = await pool.query("SELECT * FROM admins WHERE usuario = $1", ['admin']);
            if (adminRes.rows.length === 0) {
                await pool.query("INSERT INTO admins (usuario, senha) VALUES ($1, $2)", ['admin', 'admin123']);
                console.log("Admin padrão criado: admin / admin123");
            }

            // Inserir dados de exemplo se necessário (produtos, categorias, promocoes)
            const prodCount = await pool.query('SELECT COUNT(*)::int as count FROM produtos');
            if (prodCount.rows[0].count === 0) {
                ...
                for (const c of categorias) {
                    await pool.query('INSERT INTO categorias (nome, descricao, data_criacao) VALUES ($1, $2, NOW()) ON CONFLICT (nome) DO NOTHING', c);
                }

                // Inserir produtos de exemplo (mesma ordem do SQLite para manter ids previsíveis)
                const produtos = [
                ...
                for (const p of produtos) {
                    await pool.query('INSERT INTO produtos (nome, descricao, preco, quantidade, categoria, unidade, imagem, codigo_barras) VALUES ($1,$2,$3,$4,$5,$6,$7,$8)', p);
                }

                // Inserir promoções de exemplo (baseadas nos ids sequenciais criados acima)
                const promocoes = [
                    [1, 3, 68.70, 'Combo Arroz 3 pacotes'],
                    [2, 5, 52.90, 'Combo Feijão 5kg'],
                    [7, 6, 35.90, 'Meia dúzia de Pão'],
                    [15, 4, 32.90, 'Pacote Café 4un'],
                    [18, 5, 26.90, 'Kit Laranja 5kg'],
                    [27, 2, 28.90, 'Duo Queijo'],
                    [30, 3, 89.90, 'Trio de Carnes 3kg'],
                    [34, 6, 49.99, 'Pacote Papel Higiênico']
                ];
                for (const pr of promocoes) {
                    await pool.query('INSERT INTO promocoes (produto_id, quantidade, preco_combo, descricao) VALUES ($1,$2,$3,$4)', pr);
                }

                console.log('Produtos, categorias e promoções de exemplo inseridos no Postgres');
            }
    ...
    const sqliteDb = new sqlite3.Database(path.join(__dirname, '..', 'database', 'ecommerce.db'));

    db = sqliteDb;

    // Criar tabelas (SQLite) - manter comportamento anterior
    db.serialize(() => {
    // Clientes
